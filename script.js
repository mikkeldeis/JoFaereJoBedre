...
const questions = [
  ...
  {
    text: "Disney og Pixar film i et ord", // med flest stemmer
    answers: {
      "askepot": 39,
      "frost": 31,
      "op": 25,
...
function endGame() {
  ...
  const winningTeamIndex = scores.indexOf(lowestScore);
  const winningTeam = teamnames[winningTeamIndex];
  winnerText.innerText = `Vinderholdet er ${winningTeam} med ${lowestScore} point!`;
  winners[winningTeamIndex].style.display = "block";

  // Show scores of remaining teams
  let remainingTeamsText = "Øvrige hold:\n";
  const remainingTeamsContainer = document.createElement("div");
  remainingTeamsContainer.style.display = "flex";
  remainingTeamsContainer.style.flexDirection = "row";
  remainingTeamsContainer.style.justifyContent = "center";
  remainingTeamsContainer.style.marginTop = "20px";

  for (let i = 0; i < teamnames.length; i++) {
    if (i !== winningTeamIndex) {
      remainingTeamsText += `${teamnames[i]}: ${scores[i]} point\n`;
      winners[i].style.display = "block"; // Show picture of other teams
      winners[i].style.width = "150px"; // Make the pictures smaller
      winners[i].style.height = "150px"; // Make the pictures smaller
      remainingTeamsContainer.appendChild(winners[i]);
    }
  }
  const remainingTeamsElement = document.createElement("div");
  remainingTeamsElement.innerText = remainingTeamsText;
  remainingTeamsElement.style.marginTop = "20px";
  winnerContainer.appendChild(remainingTeamsElement);
  winnerContainer.appendChild(remainingTeamsContainer);
}
